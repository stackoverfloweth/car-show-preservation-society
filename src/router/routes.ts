...
export const routes = {
  ...
  login: () => ({ name: 'auth.login' }) as const,
  signup: () => ({ name: 'auth.signup' }) as const,
  accept: (token: string) => ({ name: 'auth.accept/:token', params: { token } }) as const,
  recover: () => ({ name: 'auth.recover' }) as const,
